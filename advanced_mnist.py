...
# define our our github URLs
LOGDIR = '/tmp/mnist_tutorial/'
GITHUB_URL ='https://github.com/mamcgrath/TensorBoard-TF-Dev-Summit-Tutorial/blob/master/'

# download and read the data
mnist = tf.contrib.learn.datasets.mnist.read_data_sets(train_dir=LOGDIR + 'data', one_hot = True)

# get a sprite labels firle for the embedding projector
...
def conv_layer(input, size_in, size_out, name="conv"):
    # tf.name_scope creates namespace for operator in the default graph, places into group, easier to read
    # A graph maintains a stack of name scopes, A 'with name_scope(...):'
    # statements pushes a new name onto the stack for the lifetime of the context
    ...
    with tf.name_scope(name):
        ...
        # we can collect this data by attaching tf.summary.histogram ops to the gradient outputs and to the variable that
        #holds weights, respectively
        # visualize the distribution of weights and biases
...
def mnist_model(learning_rate, use_two_conv, use_two_fc, hparam):
    ...
    # setup placeholders and reshape the data for the data (images)
    ...
    # flatten the array into a vector of 28*28 = 784 numbers
    ...
    with tf.name_scope("accurace"):
        # returns the index with the largest value across axes of a tensor
        correct_predicion = tf.equal(tf.argmax(logits,1), tf.argmax(y, 1))
        # cast a tensor to a new tyope
        accuracy = tf.reduce_mean(tf.cast(correct_predicion, tf.float32))
        # save that single number
        tf.summary.scalar("accuracy", accuracy)

    # merge them all so one can write to disk, more comp efficient
    summ = tf.summary.merge_all()

    # initialize embedding matrix as 0s
    ...
    writer = tf.summary.FileWriter(LOGDIR +hparam, sess.graph)
...
def make_hparas_string(learning_rate, use_two_conv, use_two_fc):
    conv_param = "conv=2" if use_two_conv else "conv=1"
    fc_param = "fc=2" if use_two_fc else "fc=1"

    return "le_%.0E, %s, %s"%(learning_rate, conv_param, fc_param)

def main():
    # we can try adding some more learning rates
    for learning_rate in [1E-4]:

        # includes "False" as a value to try different model architectures
        for use_two_fc in [True]:
            for use_two_conv in [True]:
                # construct a hyperparameter string for each one (example: le_1E-4, fc=2, conv=2)
                hparam = make_hparas_string(learning_rate, use_two_conv, use_two_fc)
                print('Starting run for %s'%hparam)

                # Actually run with the new settings
                mnist_model(learning_rate, use_two_conv, use_two_fc, hparam)
