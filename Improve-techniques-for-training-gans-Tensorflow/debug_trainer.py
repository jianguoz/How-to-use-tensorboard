...
class Trainer(object):
    def __init__(self,
        ...
        self.checkpoints_sec = 600 # 10 mins
        ...
        self.supervisor = tf.train.Supervisor(
            ...
            save_model_secs=self.checkpoints_sec,
            global_step=self.global_step
        )
        ...
        if self.ckpt_path is not None:
            ...
            log.info("Loaded the pretrained parameters from the provided checkpoint path")


    def train(self):
        log.infov("Training Srarts!")
        ...
        for s in xrange(max_steps):
            ...
            if s % output_save_step == 0:
                log.infov("Saved checkpoints at %d", s)
    ...
    def run_single_step(self, batch, step=None, is_train=True):
        ...
        [step, accuracy, summary, d_loss, g_loss, s_loss, all_preds, all_targets, g_img] = fetch_values[:9]

        _end_time = time.time()

        return step, accuracy, summary, d_loss, g_loss, s_loss, (_end_time - _start_time), all_preds, all_targets, g_img

    def run_test(self, batch, is_train=False, repeat_time=8):
        batch_chunk = self.session.run(batch)

        [step, accuracy, all_preds, all_tragets] = self.session.run(
        ...
        return accuracy, all_preds, all_tragets
