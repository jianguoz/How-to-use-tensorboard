...
class Model(object):
    def __init__(self, config,
                 debug_information=False, is_train=True):
        ...
        self.image = tf.placeholder(tf.float32, shape=[self.batch_size, self.input_height,
                                                       self.input_width,self.c_dim], name="image")
        ...
        self.reconv_weight = tf.placeholder_with_default(tf.cast(1.0,tf.float32),[])

        tf.summary.scalar("loss/recon_weight", self.reconv_weight)
    ...
    def get_feed_dict(self, batch_chunk, step=None, is_training=None):
        ...
        # weight annealing
        if step is not None:
            fd[self.reconv_weight] = min(max(0, (1500-step)/1500), 1.0)*10
    ...
    def build(self, is_train = True):
        ...
        def build_loss(d_real, d_real_logits, d_fake, d_fake_logits, label, real_image, fake_image):
            ...
            # Weight annealing
            g_loss += tf.reduce_mean(huber_loss(real_image, fake_image))*self.reconv_weight
        ...
        # G takes random noise and tries to generate images [B, h, w, c]
        ...
        # D take images as input and tries to output class label [B, n+1]
        def D(img, scope = 'Discriminator', reuse = True):
            with tf.variable_scope(scope, reuse=reuse) as scope:
                ...
                d_3 = conv2d(d_2, conv_info[0], is_train, name='d_3_conv')
                d_3 = slim.dropout(d_3, keep_prob=0.5, is_training=is_train, scope='d_3_conv/')
                if not reuse: log.info('{} {}'.format(scope.name, d_3))
                d_4 = slim.fully_connected(tf.reshape(d_3,[self.batch_size,-1]), np.asscalar(n+1), scope='d_4_fc',activation_fn= None)
                if not reuse: log.info('{} {}'.format(scope.name, d_4))
                output = d_4
                assert output.get_shape().as_list() == [self.batch_size, n+1], 'Discriminator dimension error'
        ...
        d_fake, d_fake_logits = D(self.fake_img, scope='Discriminator', reuse = True)
        ...
        tf.summary.scalar("loss/d_loss_real", tf.reduce_mean(self.d_loss_real))
        tf.summary.scalar("loss/d_loss_fake", tf.reduce_mean(self.d_loss_fake))
